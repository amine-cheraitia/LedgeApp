...
class PrestationApiTest extends TestCase
{
    ...
    public function test_admin_affiche_une_prestation(): void
    {
        $prestation = $this->prestation();

        $this->actingAs($this->admin)
            ->getJson("/api/v1/prestations/{$prestation->id}")
            ->assertOk()
            ->assertJsonPath('data.id', $prestation->id)
            ->assertJsonPath('data.designation', 'Assistance comptable');
    }
