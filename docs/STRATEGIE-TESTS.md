...
|---|---|---|---|---|
| **Backend** (Laravel API) | PHPUnit 11 | **302** | 35 | **81.5 %** |
| **Frontend** (Vue 3 SPA) | Vitest 4 + @vue/test-utils | **751** | 49 | **94.9 %** |
| **Total** | | **1 053** | | |
...
```
                 ...
                 ├─────────────────────────────┤
        Feature  │  271 tests API backend       │   HTTP réel -> BDD SQLite mémoire
        + pages  │  ~500 tests composants front │   montage réel des composants
                 ├─────────────────────────────┤
...
**Répartition** : 31 tests unitaires (`tests/Unit`) + 271 tests de fonctionnalité (`tests/Feature`).
...
**Répartition** (49 fichiers) : 24 pages · 14 composables · 7 (client API, guard router, utils) ·
