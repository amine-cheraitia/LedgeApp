...
- **Test de non-régression du rate limiting du login** : ajout de `tests/Feature/Api/LoginTest.php` verrouillant le `throttle:5,1` sur `POST /login` (6ᵉ tentative → `429`), plus la connexion valide (`200`) et les identifiants invalides (`422`). Contrôles : **286 tests backend ✓** (+3), `vue-tsc`/`vite build` ✓, **125 tests Vitest ✓**.
