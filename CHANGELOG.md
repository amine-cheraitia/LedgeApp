...
- **Test de non-régression du rate limiting du login** : ajout de `tests/Feature/Api/LoginTest.php` verrouillant le `throttle:5,1` sur `POST /login` (6ᵉ tentative → `429`), plus la connexion valide (`200`) et les identifiants invalides (`422`).
- **Page 404 (route catch-all)** : ajout d'une route attrape-tout `/:pathMatch(.*)*` → page [NotFoundPage.vue](frontend/src/pages/errors/NotFoundPage.vue) au lieu d'un `<router-view>` vide sur URL inconnue. Standalone (visible dans toutes les zones : visiteur, client, staff), RGAA (skip link, `role="alert"`, `aria-label`), bouton de retour adapté à l'état de session (`/portail`, `/`, ou `/login`). Pendant de la page `/acces-refuse` existante. Test de non-régression ajouté ([router.test.ts](frontend/src/__tests__/router.test.ts)).
- **Thème clair/sombre appliqué dès le bootstrap** : la classe `.app-dark` était posée au niveau module de [layout.ts](frontend/src/layout/composables/layout.ts) — importé seulement par l'AppLayout — donc les pages standalone (login, 404) chargées à froid restaient en **clair** même avec le dark mode actif. Extraction d'un `applyStoredTheme()` appelé dans [main.ts](frontend/src/main.ts) avant le montage → toutes les routes respectent la préférence dès le premier rendu.
- **Chiffres du tableau de bord en JetBrains Mono** : les valeurs KPI (admin + collaborateur) passent en police monospace `--ledge-ff-mono` (rendu tabulaire, cohérent avec le grand « 404 »).
- Contrôles : **286 tests backend ✓** (+3), `vue-tsc`/`vite build` ✓, **127 tests Vitest ✓** (+4).
