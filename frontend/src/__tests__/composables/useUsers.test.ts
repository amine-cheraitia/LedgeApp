...
describe('useUsers — deleteUser', () => {
  it('supprime, notifie le succes puis refetch', async () => {
    mockDelete.mockResolvedValue(undefined)
    const u = useUsers()

    await u.deleteUser(7)

    expect(mockDelete).toHaveBeenCalledWith(7)
    expect(toastAdd).toHaveBeenCalledWith(
      expect.objectContaining({ severity: 'success', detail: 'Utilisateur supprime.' }),
    )
    expect(mockGetAll).toHaveBeenCalledOnce()
  })

  it('avale l erreur (pas de throw) et affiche le message API', async () => {
    mockDelete.mockRejectedValue({ response: { data: { message: 'Suppression impossible.' } } })
    const u = useUsers()

    await u.deleteUser(7) // ne doit pas rejeter

    expect(toastAdd).toHaveBeenCalledWith(
      expect.objectContaining({ severity: 'error', detail: 'Suppression impossible.' }),
    )
    expect(mockGetAll).not.toHaveBeenCalled()
  })
})

describe('useUsers — pagination et recherche', () => {
  it('onPage passe a la page demandee (index 0 -> page 1)', async () => {
    const u = useUsers()

    u.onPage({ page: 2 })
    await vi.waitFor(() => expect(mockGetAll).toHaveBeenCalled())

    expect(u.filters.value.page).toBe(3)
    expect(mockGetAll).toHaveBeenCalledWith(expect.objectContaining({ page: 3 }))
  })

  it('onSearch enregistre la recherche et revient page 1', async () => {
    const u = useUsers()
    u.filters.value.page = 4

    u.onSearch('benali')
    await vi.waitFor(() => expect(mockGetAll).toHaveBeenCalled())

    expect(u.filters.value.page).toBe(1)
    expect(mockGetAll).toHaveBeenCalledWith(expect.objectContaining({ search: 'benali', page: 1 }))
  })

  it('onSearch avec chaine vide envoie search=undefined', async () => {
    const u = useUsers()
