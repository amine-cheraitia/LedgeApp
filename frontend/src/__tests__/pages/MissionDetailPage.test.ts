...
describe('MissionDetailPage — modification de tâche', () => {
  it('pré-remplit le dialog depuis la tâche puis enregistre les modifications', async () => {
    mockTacheUpdate.mockResolvedValue({ data: makeTaches()[0] })
    const { wrapper } = await mountDetail()

    await findButton(wrapper, 'Modifier la tâche Collecte des pièces')!.trigger('click')
    await flushPromises()

    const titreInput = wrapper.find('#e-titre')
    expect((titreInput.element as HTMLInputElement).value).toBe('Collecte des pièces')
    expect((wrapper.find('#e-desc').element as HTMLTextAreaElement).value).toBe('Récupérer les justificatifs')

    await titreInput.setValue('Collecte des pièces — relue')
    await wrapper.find('form').trigger('submit')
    await flushPromises()

    expect(mockTacheUpdate).toHaveBeenCalledWith(5, 10, expect.objectContaining({
      titre: 'Collecte des pièces — relue',
      statut: 'en_cours',
      priorite: 3,
      date_debut: '2026-02-01',
      date_echeance: '2026-02-15',
    ...
    expect(toastAdd).toHaveBeenCalledWith(
      expect.objectContaining({ severity: 'success', detail: 'Tâche modifiée.' }),
    )
    expect(wrapper.find('#e-titre').exists()).toBe(false)
    expect(mockTachesGetAll).toHaveBeenCalledTimes(2)
  })

  it("laisse le dialog ouvert avec un toast d'erreur si la modification échoue", async () => {
    mockTacheUpdate.mockRejectedValue(new Error('500'))
    const { wrapper } = await mountDetail()

    await findButton(wrapper, 'Modifier la tâche Saisie comptable')!.trigger('click')
    await flushPromises()
    await wrapper.find('form').trigger('submit')
    await flushPromises()

    expect(mockTacheUpdate).toHaveBeenCalledWith(5, 11, expect.anything())
    expect(toastAdd).toHaveBeenCalledWith(
...
describe('MissionDetailPage — suppression de tâche', () => {
  it('demande confirmation puis supprime la tâche et recharge la liste', async () => {
    mockTacheDelete.mockResolvedValue(undefined)
    const { wrapper } = await mountDetail()

    await findButton(wrapper, 'Supprimer la tâche Collecte des pièces')!.trigger('click')

    expect(confirmRequire).toHaveBeenCalledOnce()
    const config = confirmRequire.mock.calls[0][0]
    expect(config.message).toContain('Collecte des pièces')
    expect(config.acceptLabel).toBe('Supprimer')

    await config.accept()
    await flushPromises()

    expect(mockTacheDelete).toHaveBeenCalledWith(5, 10)
    expect(toastAdd).toHaveBeenCalledWith(
      expect.objectContaining({ severity: 'success', detail: 'Tâche supprimée.' }),
    )
    expect(mockTachesGetAll).toHaveBeenCalledTimes(2)
  })

  it("affiche le message d'erreur de l'API, ou un message par défaut, si la suppression échoue", async () => {
    const { wrapper } = await mountDetail()
    const vm = wrapper.vm as any
    const taches = makeTaches()

    // message métier renvoyé par le backend (ex: commentaires associés)
    mockTacheDelete.mockRejectedValueOnce({ response: { data: { message: 'Tâche avec commentaires associés.' } } })
    await vm.deleteTache(taches[0])
    expect(toastAdd).toHaveBeenCalledWith(
      expect.objectContaining({ severity: 'error', detail: 'Tâche avec commentaires associés.' }),
    )

    // erreur sans réponse structurée -> message générique
    mockTacheDelete.mockRejectedValueOnce(new Error('network'))
    await vm.deleteTache(taches[1])
    expect(toastAdd).toHaveBeenCalledWith(
      expect.objectContaining({ severity: 'error', detail: 'Impossible de supprimer la tâche.' }),
    )
  })
})

describe('MissionDetailPage — navigation', () => {
  it('navigue vers le détail de tâche puis revient à la liste des missions', async () => {
    const { wrapper, router } = await mountDetail()

    await findButton(wrapper, 'Voir la tâche Collecte des pièces')!.trigger('click')
    await flushPromises()
    expect(router.currentRoute.value.name).toBe('tache-detail')
    expect(router.currentRoute.value.params).toMatchObject({ id: '5', tacheId: '10' })

    await findButton(wrapper, 'Retour aux missions')!.trigger('click')
    await flushPromises()
    expect(router.currentRoute.value.name).toBe('missions')
  })
})
