...
describe('PlanningCalendarPage — collaborateur', () => {
  it('voit uniquement SES taches en lecture seule, sans onglet Équipe', async () => {
    const { wrapper } = await mountPage(PlanningCalendarPage, { role: 'collaborateur' })

    expect(wrapper.text()).toContain('Mes tâches')
    expect(wrapper.text()).not.toContain('Équipe')
    expect(mockUsersGetAll).not.toHaveBeenCalled() // pas de filtre collaborateur
    expect((wrapper.vm as any).calendarOptions.editable).toBe(false)

    // legende des priorites (RGAA : region etiquetee)
    expect(wrapper.find('[aria-label="Légende des priorités"]').exists()).toBe(true)
    expect(wrapper.text()).toContain('Faible')
    expect(wrapper.text()).toContain('Urgente')

    await triggerDatesSet(wrapper)
    const events = fakeApi.addEventSource.mock.calls.at(-1)![0]
    expect(events.map((e: any) => e.id)).toEqual(['tache-11']) // ses taches, pas les missions
  })
})
