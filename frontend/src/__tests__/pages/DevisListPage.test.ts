...
describe('DevisListPage — cycle de vie', () => {
  it('envoie le devis brouillon apres confirmation', async () => {
    mockDevisEnvoyer.mockResolvedValue({ data: { ...devisList[0], statut: 'envoye' } })
    const { wrapper } = await mountPage(DevisListPage)

    await findButton(wrapper, 'Envoyer le devis par mail au client')!.trigger('click')

    expect(confirmRequire).toHaveBeenCalledOnce()
    const config = confirmRequire.mock.calls[0][0]
    expect(config.message).toContain('DV2026-001')
    expect(config.acceptLabel).toBe('Envoyer')

    await config.accept()
    await flushPromises()

    expect(mockDevisEnvoyer).toHaveBeenCalledWith(1)
    expect(toastAdd).toHaveBeenCalledWith(
      expect.objectContaining({ severity: 'success', detail: 'Devis envoye au client par mail.' }),
    )
  })

  it('accepte un devis envoye puis refetch la liste', async () => {
    mockDevisAccepter.mockResolvedValue({ data: { ...devisList[1], statut: 'accepte' } })
    const { wrapper } = await mountPage(DevisListPage)
    mockDevisGetAll.mockClear()

    await findButton(wrapper, 'Accepter')!.trigger('click')
    await flushPromises()

    expect(mockDevisAccepter).toHaveBeenCalledWith(2)
  ...
  it('refuse un devis envoye', async () => {
    mockDevisRefuser.mockResolvedValue({ data: { ...devisList[1], statut: 'refuse' } })
    const { wrapper } = await mountPage(DevisListPage)

    await findButton(wrapper, 'Refuser')!.trigger('click')
    await flushPromises()

    expect(mockDevisRefuser).toHaveBeenCalledWith(2)
    expect(toastAdd).toHaveBeenCalledWith(expect.objectContaining({ severity: 'success', detail: 'Devis refuse.' }))
  })

  it('supprime un devis brouillon apres confirmation', async () => {
    mockDevisDelete.mockResolvedValue(undefined)
    const { wrapper } = await mountPage(DevisListPage)
    ...
    expect(mockDevisDelete).toHaveBeenCalledWith(1)
    expect(toastAdd).toHaveBeenCalledWith(expect.objectContaining({ severity: 'success', detail: 'Devis supprime.' }))
    expect(mockDevisGetAll).toHaveBeenCalled()
  })
})

describe('DevisListPage — conversion en mission (admin)', () => {
  it('ouvre le dialog de conversion et cree la mission avec dates + collaborateurs', async () => {
    mockDevisConvertir.mockResolvedValue({ data: { id: 50, reference: 'M2026-001' } })
    const { wrapper } = await mountPage(DevisListPage)

    await findButton(wrapper, 'Convertir en mission')!.trigger('click')
    await flushPromises()

    const vm = wrapper.vm as any
    expect(vm.conversionVisible).toBe(true)
    expect(wrapper.text()).toContain('Convertir DV2026-003 en mission')

    vm.conversionForm.date_debut = new Date(YEAR, 6, 1)
    vm.conversionForm.date_fin = new Date(YEAR, 11, 31)
    vm.conversionForm.collaborateur_ids = [4, 5]
    await flushPromises()

    await wrapper.find('form').trigger('submit')
    await flushPromises()

    expect(mockDevisConvertir).toHaveBeenCalledWith(3, {
      date_debut: `${YEAR}-07-01`,
      date_fin: `${YEAR}-12-31`,
      collaborateur_ids: [4, 5],
    })
    expect(vm.conversionVisible).toBe(false)
    expect(toastAdd).toHaveBeenCalledWith(
      expect.objectContaining({ severity: 'success', detail: 'Mission creee depuis le devis.' }),
    )
  })

  it('sans date de fin : envoie date_fin null ; sans date de debut : ne soumet pas', async () => {
    mockDevisConvertir.mockResolvedValue({ data: { id: 51 } })
    const { wrapper } = await mountPage(DevisListPage)
    const vm = wrapper.vm as any

    vm.openConversion(devisList[2])
    await vm.onConvertir() // date_debut manquante
    expect(mockDevisConvertir).not.toHaveBeenCalled()

    vm.conversionForm.date_debut = new Date(YEAR, 6, 1)
    await vm.onConvertir()

    expect(mockDevisConvertir).toHaveBeenCalledWith(3, expect.objectContaining({ date_fin: null, collaborateur_ids: [] }))
  })

  it('garde le dialog de conversion ouvert si la conversion echoue', async () => {
    mockDevisConvertir.mockRejectedValue(new Error('422'))
    const { wrapper } = await mountPage(DevisListPage)
    const vm = wrapper.vm as any
    ...
    expect(vm.conversionVisible).toBe(true)
    expect(vm.converting).toBe(false)
  })
})

describe('DevisListPage — PDF', () => {
  it('telecharge le PDF du devis sous le nom devis-{numero}.pdf', async () => {
    mockDevisGetPdf.mockResolvedValue(new Blob(['%PDF']))
    const { wrapper } = await mountPage(DevisListPage)
    ...
    // Premier bouton PDF = premier devis non-brouillon (id 2)
    await findButton(wrapper, 'Telecharger le PDF')!.trigger('click')
    await flushPromises()
    createSpy.mockRestore()

    expect(mockDevisGetPdf).toHaveBeenCalledWith(2)
    expect(anchor.download).toBe('devis-DV2026-002.pdf')
