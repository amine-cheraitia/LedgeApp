...
describe('SecretaireDashboardSection — top débiteurs', () => {
  it('classe les débiteurs avec lien vers la fiche entreprise et montant en DA', async () => {
    const { wrapper } = await mountSection(makeStats())

    expect(wrapper.text()).toContain('Top 5 clients débiteurs')
    expect(wrapper.text()).toContain('SARL Atlas Conseil')
    expect(wrapper.text()).toContain('EURL Sahara Btp')

    const lien = wrapper.find('a[aria-label="Fiche de SARL Atlas Conseil"]')
    expect(lien.exists()).toBe(true)
    expect(lien.attributes('href')).toBe('/entreprises/7')
    // barre proportionnelle au max debiteur
    expect((wrapper.vm as any).maxDebiteur).toBe(800000)
  })

  it('affiche un état vide sans débiteur', async () => {
    const { wrapper } = await mountSection(emptyStats)

    expect(wrapper.text()).toContain('Aucun client débiteur.')
    expect(wrapper.find('.reco-rank__item').exists()).toBe(false)
  })
})

describe('SecretaireDashboardSection — table créances urgentes', () => {
  it('formate numéro, échéance, statut (avec repli) et marque les lignes en retard', async () => {
    const { wrapper } = await mountSection(makeStats())
    const text = wrapper.text()

    expect(text).toContain('Créances urgentes')
    expect(text).toContain('FF2026-001')
    expect(text).toContain('FF2026-002')
    // statut connu mappe, statut inconnu affiche brut
    expect(text).toContain('En attente')
    expect(text).toContain('inconnu')
    // date formatee fr-FR + repli em-dash pour date nulle
    expect(text).toContain('01/05/2026')
    expect(text).toContain('—')
    // ligne en retard stylee via row-class
    expect(wrapper.find('.reco-row--retard').exists()).toBe(true)
    expect(wrapper.find('.reco-echeance--retard').exists()).toBe(true)
    // bouton d acces aux creances
    expect(wrapper.text()).toContain('Voir les créances')
  })

  it('affiche un message quand aucune créance en cours', async () => {
    const { wrapper } = await mountSection(emptyStats)

    expect(wrapper.text()).toContain('Aucune créance en cours.')
    expect(wrapper.find('.reco-row--retard').exists()).toBe(false)
  })
})
