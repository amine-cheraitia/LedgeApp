...
describe('MissionListPage — navigation', () => {
  it('navigue vers le detail de la mission au clic sur l oeil', async () => {
    const { wrapper, router } = await mountPage(MissionListPage)
    const push = vi.spyOn(router, 'push').mockResolvedValue(undefined as any)

    const eyeButtons = wrapper.findAll('button').filter(b => b.attributes('aria-label') === 'Voir le détail de la mission')
    await eyeButtons[0].trigger('click')

    expect(push).toHaveBeenCalledWith({ name: 'mission-detail', params: { id: 1 } })
  })
})

describe('MissionListPage — recherche, pagination, tri', () => {
  it('debounce la recherche et ne garde que la derniere saisie', async () => {
    const { wrapper } = await mountPage(MissionListPage)
    missionsGetAll.mockClear()

    vi.useFakeTimers()
    await wrapper.find('#m-search').setValue('atl')
    await wrapper.find('#m-search').setValue('atlas')
    expect(missionsGetAll).not.toHaveBeenCalled()
    vi.advanceTimersByTime(300)
    vi.useRealTimers()
    await flushPromises()

    expect(missionsGetAll).toHaveBeenCalledTimes(1)
    expect(missionsGetAll).toHaveBeenCalledWith(expect.objectContaining({ search: 'atlas', page: 1 }))
  })

  it('pagine et trie via les evenements du tableau', async () => {
    const { wrapper } = await mountPage(MissionListPage)
    const vm = wrapper.vm as any
    missionsGetAll.mockClear()

    vm.onPage({ page: 2 })
    expect(missionsGetAll).toHaveBeenLastCalledWith(expect.objectContaining({ page: 3 }))

    vm.onSort({ sortField: 'reference', sortOrder: 1 })
    expect(missionsGetAll).toHaveBeenLastCalledWith(
      expect.objectContaining({ sort_field: 'reference', sort_direction: 'asc', page: 1 }),
    )

    // sortField non-string (removableSort) -> champ efface, direction desc
    vm.onSort({ sortField: null, sortOrder: -1 })
    const last = missionsGetAll.mock.calls.at(-1)![0]
    expect(last.sort_field).toBeUndefined()
    expect(last.sort_direction).toBe('desc')
  })

  it("efface le filtre exercice -> refetch sans exercice_id", async () => {
    const { wrapper } = await mountPage(MissionListPage)
    const vm = wrapper.vm as any
    missionsGetAll.mockClear()

    vm.exerciceSelectionne = undefined
    await flushPromises()

    expect(missionsGetAll).toHaveBeenCalledTimes(1)
    expect(missionsGetAll.mock.calls.at(-1)![0].exercice_id).toBeUndefined()
  })
})
