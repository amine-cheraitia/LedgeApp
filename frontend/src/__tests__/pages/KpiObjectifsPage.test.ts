...
describe('KpiObjectifsPage — sauvegarde des objectifs', () => {
  it('envoie un upsert par objectif renseigné puis recharge silencieusement', async () => {
    mockUpsert.mockResolvedValue({ data: { id: 1, type: 'ca_ht', valeur: 2000000 } })
    const { wrapper } = await mountPage(KpiObjectifsPage)

    const editing = (wrapper.vm as any).editing
    editing['10-ca_ht'] = 2000000
    editing['10-missions_cloturees'] = 5

    await findButton(wrapper, 'Sauvegarder les objectifs de ali benali')!.trigger('click')
    await flushPromises()

    expect(mockUpsert).toHaveBeenCalledTimes(2)
    expect(mockUpsert).toHaveBeenCalledWith({ user_id: 10, exercice_id: 1, type: 'ca_ht', valeur: 2000000 })
    expect(mockUpsert).toHaveBeenCalledWith({ user_id: 10, exercice_id: 1, type: 'missions_cloturees', valeur: 5 })
    expect(toastAdd).toHaveBeenCalledWith(
      expect.objectContaining({ severity: 'success', detail: 'Objectifs mis à jour.' }),
    )
    // refetch silencieux apres sauvegarde (montage + rechargement)
    expect(mockGetKpi).toHaveBeenCalledTimes(2)
    expect((wrapper.vm as any).saving).toBeNull()
  })

  it('affiche un toast erreur si la sauvegarde échoue, sans recharger', async () => {
    mockUpsert.mockRejectedValue(new Error('422'))
    const { wrapper } = await mountPage(KpiObjectifsPage)
  ...
  it('ne fait aucun appel quand aucun objectif n est renseigné', async () => {
    const { wrapper } = await mountPage(KpiObjectifsPage)

    // Sara n a aucun objectif ni saisie
    await findButton(wrapper, 'Sauvegarder les objectifs de Sara Khelifi')!.trigger('click')
    await flushPromises()

    expect(mockUpsert).not.toHaveBeenCalled()
  })

  it('ne fait rien si aucun exercice n est sélectionné', async () => {
    const { wrapper } = await mountPage(KpiObjectifsPage)
    const vm = wrapper.vm as any

    vm.exerciceId = null
    await vm.sauvegarder(makeCollabs()[0])

    expect(mockUpsert).not.toHaveBeenCalled()
  })

  it('la recharge silencieuse avale les erreurs sans toast', async () => {
    mockUpsert.mockResolvedValue({ data: { id: 1, type: 'ca_ht', valeur: 1 } })
    const { wrapper } = await mountPage(KpiObjectifsPage)
    ...
    // seul le toast succes de sauvegarde, pas d erreur pour le refetch silencieux
    expect(toastAdd).toHaveBeenCalledWith(expect.objectContaining({ severity: 'success' }))
    expect(toastAdd).not.toHaveBeenCalledWith(expect.objectContaining({ severity: 'error' }))
  })
})

describe('KpiObjectifsPage — changement d exercice', () => {
  it('recharge les KPI pour l exercice choisi', async () => {
    const { wrapper } = await mountPage(KpiObjectifsPage)
    const vm = wrapper.vm as any
    ...
    expect(mockGetKpi).toHaveBeenLastCalledWith(2)
  })
})
