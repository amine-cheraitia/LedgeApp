...
import PortailDocumentsPage from '@/pages/portail/PortailDocumentsPage.vue'
...
// ─────────────────────────────────────────────────────────────────────────────
describe('PortailDocumentsPage', () => {
  function mockDocuments(list: Mission[], pdf: 'ok' | 'ko' = 'ok') {
    mockGet.mockImplementation((url: string) => {
      if (url === '/portail/documents') return Promise.resolve({ data: { data: list } })
      if (url.endsWith('/pdf')) {
        return pdf === 'ok' ? Promise.resolve({ data: 'PDF-BYTES' }) : Promise.reject(new Error('500'))
      }
      return Promise.reject(new Error(`GET inattendu: ${url}`))
    })
  }

  it("affiche l'etat vide quand aucune mission n'a de document", async () => {
    mockDocuments([])
    const { wrapper } = await mountPage(PortailDocumentsPage, { role: 'client' })

    expect(mockGet).toHaveBeenCalledWith('/portail/documents')
    expect(wrapper.text()).toContain('Aucun document disponible pour le moment.')
    expect(wrapper.find('.empty-state').attributes('role')).toBe('status')
  })

  it('affiche conventions et mandats avec leurs numeros et dates', async () => {
    mockDocuments([
      makeMission(), // convention + mandat, date_fin null -> '-'
      makeMission({ id: 2, reference: 'M2026-002', mandat_numero: null }),
    ])
    const { wrapper } = await mountPage(PortailDocumentsPage, { role: 'client' })

    expect(wrapper.text()).toContain('Mes documents')
    expect(wrapper.text()).toContain('Convention de prestation')
    expect(wrapper.text()).toContain("Mandat d'acceptation")
    expect(wrapper.text()).toContain('CV2026-001')
    expect(wrapper.text()).toContain('MD2026-001')
    expect(wrapper.text()).toContain('M2026-002')
    // date_debut formatee + date_fin null -> '-'
    expect(wrapper.text()).toContain('2026')
    expect(wrapper.text()).toContain('-')
    // La mission 2 n'a pas de mandat : 3 boutons Telecharger seulement
    const telechargers = wrapper.findAll('button').filter(b => b.text().includes('Télécharger'))
    expect(telechargers).toHaveLength(3)
  })

  it('affiche un toast erreur si le chargement echoue', async () => {
    mockGet.mockRejectedValue(new Error('500'))
    await mountPage(PortailDocumentsPage, { role: 'client' })
  ...
  it('telecharge la convention puis le mandat en PDF', async () => {
    mockDocuments([makeMission()])
    const { wrapper } = await mountPage(PortailDocumentsPage, { role: 'client' })

    const { anchors, restore } = captureAnchors()
    await findButton(wrapper, 'Télécharger la convention CV2026-001')!.trigger('click')
    await flushPromises()
    expect(mockGet).toHaveBeenCalledWith('/portail/documents/1/convention/pdf', { responseType: 'blob' })
    expect(anchors.at(-1)!.download).toBe('convention-CV2026-001.pdf')

    await findButton(wrapper, 'Télécharger le mandat MD2026-001')!.trigger('click')
    await flushPromises()
    expect(mockGet).toHaveBeenCalledWith('/portail/documents/1/mandat/pdf', { responseType: 'blob' })
    expect(anchors.at(-1)!.download).toBe('mandat-MD2026-001.pdf')
    restore()
  })

  it('ne lance aucun appel si la mission n a pas de numero de document', async () => {
    mockDocuments([makeMission()])
    const { wrapper } = await mountPage(PortailDocumentsPage, { role: 'client' })
    const vm = wrapper.vm as never as {
      telechargerConvention: (m: Mission) => Promise<void>
      telechargerMandat: (m: Mission) => Promise<void>
    }
    const appelsAvant = mockGet.mock.calls.length

    await vm.telechargerConvention(makeMission({ convention_numero: null }))
    await vm.telechargerMandat(makeMission({ mandat_numero: null }))

    expect(mockGet.mock.calls.length).toBe(appelsAvant)
  })

  it('affiche un toast erreur si le telechargement convention/mandat echoue', async () => {
    mockDocuments([makeMission()], 'ko')
    const { wrapper } = await mountPage(PortailDocumentsPage, { role: 'client' })
    ...
    expect(toastAdd).toHaveBeenCalledWith(expect.objectContaining({
      severity: 'error',
      detail: 'Impossible de télécharger la convention.',
    }))
    ...
    expect(toastAdd).toHaveBeenCalledWith(expect.objectContaining({
      severity: 'error',
      detail: 'Impossible de télécharger le mandat.',
    }))
  })
})

// ─────────────────────────────────────────────────────────────────────────────
describe('portailApi — module', () => {
  it('me() appelle GET /portail/me et deballe la reponse', async () => {
    mockGet.mockResolvedValue({ data: { data: { id: 7, name: 'Client' } } })

    const res = await portailApi.me()

    expect(mockGet).toHaveBeenCalledWith('/portail/me')
    expect(res.data).toEqual({ id: 7, name: 'Client' })
  })

  it('rapportMissionPdfUrl construit une URL sans appel HTTP', () => {
    expect(portailApi.rapportMissionPdfUrl(42)).toBe('/api/v1/portail/missions/42/rapport/pdf')
