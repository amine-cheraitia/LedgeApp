...
function updateColors(type: string, color: any) {
  if (type === 'primary') layoutConfig.primary = color.name
  else if (type === 'surface') layoutConfig.surface = color.name
...
function onPresetChange() {
  ...
  ;($t() as any).preset(presetValue).preset(getPresetExt()).surfacePalette(surfacePalette).use({ useDefaultOptions: true })
}
</script>
...
<template>
  ...
  >
    <div class="flex flex-col gap-4">
      ...
      <div class="flex flex-col gap-2">
        <span class="text-sm text-muted-color font-semibold">Mode menu</span>
